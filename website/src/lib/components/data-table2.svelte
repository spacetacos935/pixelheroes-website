<script context="module" lang="ts">
	...
	export type { SortKey } from 'svelte-headless-table/plugins';

	export type Column<TItem, TValue = any> = {
		header: string;
		id?: string;
		plugins?: {
			sort?: SortByColumnOptions;
			filter?: TableFilterColumnOptions<TItem>;
		};
	} & (
		| {
				accessor: keyof TItem;
				cell?: (context: { value: TItem[keyof TItem] }) => any;
		  }
		| {
				accessor: (item: TItem) => TValue;
				cell?: (context: { value: TValue }) => any;
		  }
	);
</script>
...
<script lang="ts">
	...
	export let defaultSortyKeys: SortKey[] = [];
	...
	let viewModel = table.createViewModel(tableColumns);
	...
	async function fetchData() {
		...
		const { total, data } = await fetchFunction(
			$pageIndex,
			$sortKeys.length > 0 ? $sortKeys[0].id : defaultSortyKeys[0].id,
			$sortKeys.length > 0 ? $sortKeys[0].order : defaultSortyKeys[0].order,
			$filterValue
		);
	...
	let debouncedScroll = debounce(() => {
		if (window.scrollY > container.getBoundingClientRect().top + window.scrollY) {
...
<div bind:this={container} class="relative scroll-mt-32 {$$restProps.class || ''}">
	{#if loading}
		<div
