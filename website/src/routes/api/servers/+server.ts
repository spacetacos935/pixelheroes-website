...
export const GET: RequestHandler = async ({ url }) => {
	...
	const countQuery = supabase
		...
		.eq('region', region);
	const fetchQuery = supabase.from('server_statistics').select().eq('region', region);
	...
	const [{ count, error: countError }, { data: servers, error: fetchError }] = await Promise.all([
		countQuery,
		fetchQuery.range(skip, skip + take - 1).order(sortBy === 'id' ? 'serverId' : sortBy, {
	...
	return json({ total: count, servers });
};
